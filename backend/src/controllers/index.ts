...
import { uploadBase64ImageToCloudinary, uploadBase64PdfToCloudinary } from "../utils";
import Blog from "../models/blog.model";

export const upload = async (req: Request, res: Response) => {
  try {
    const { title, category, description, blogImage, keywords, tags, author, content, } = req.body

    if (!title || !category || !description || !blogImage || !keywords || !tags || !author || !content) return res.status(500).json({ success: false, message: 'Please fill all fields.' })

    const imageLink = await uploadBase64ImageToCloudinary(blogImage, 'blog')

    const blog = await Blog.create({
      ...req.body, image: imageLink, keywords: keywords.split(","), tags: tags.split(","),
    })

    return res.status(201).json({
      success: true, data: 'Blog created successfully.'
    })
  } catch (error) {
    console.log(error)
...
export const updateBlog = async (req: Request, res: Response) => {
  try {
    ...
    await Blog.findByIdAndUpdate(id, {
      ...req.body, image: imageLink, keywords: keywords.split(","), tags: tags,
    })
...
export const uploadPdf = async (req: Request, res: Response) => {
  try {
    ...
    if (!pdf) return res.status(500).json({ success: false, message: 'Please provide a pdf.' })
